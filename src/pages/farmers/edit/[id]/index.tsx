...
import { getFarmerById, updateFarmerById } from 'apiSdk/farmers';
...
function FarmerEditPage() {
  ...
  const handleSubmit = async (values: FarmerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateFarmerById(id, values);
      mutate(updated);
      resetForm();
      router.push('/farmers');
    } catch (error) {
  ...
  const formik = useFormik<FarmerInterface>({
    initialValues: data,
